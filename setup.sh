...
echo -e "${BLUE}                                    https://github.com/Discordmodsbers/DorkSearcher.git ${NC}"
...
echo -n -e "Dork >> "
read choice
INSTALL_DIR="/usr/share/doc/dorsearch"
BIN_DIR="/usr/bin/"
if [ $choice == 1 ] || [ $choice == 2 ]; then
	...
	if [[ $? == 0 ]]; then
	    ...
	    if [ -d "$INSTALL_DIR" ]; then
	        echo "[!] A Directory Dorksearch Was Found.. Do You Want To Replace It ? [y/n]:" ;
	        read input
	        if [ "$input" = "y" ]; then
        ...
        echo "[✔] Installing ...\n";
        sudo git clone https://github.com/Discordmodsbers/DorkSearcher.git "$INSTALL_DIR";
        echo "#!/bin/bash
        python3 $INSTALL_DIR/DorkSearcher.py" '${1+"$@"}' > dorksearch;
        sudo chmod +x dorksearch;
        sudo cp dorksearch /usr/bin/ && rm dorksearch;

        echo "\n[✔] Trying to installing Requirements ..."
        if [ $choice == 1 ]; then
            sudo pip3 install lolcat boxes flask requests google argparse pprint bs4 colorama
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        echo -e $ORANGE "       [+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo            "       [+]                                                             [+]"
        echo -e $ORANGE "       [+]     ✔✔✔ Now Just Type In Terminal (dorksearch) ✔✔✔         [+]"
        echo            "       [+]                                                             [+]"
        echo -e $ORANGE "       [+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
